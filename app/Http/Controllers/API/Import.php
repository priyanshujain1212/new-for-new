...
class Import extends Controller
{
    public function index(Request $request){
        try {
            ...
            switch($import_type){
                ...
                case "CUSTOMER":
                    if(!check_access(['A_UPLOAD_CUSTOMER'], true)){
                        throw new Exception("Invalid request", 400);
                    }
                break;
                case "STORE":
    ...
    public function forward_import_request($import_type, $import_file){
        ...
        switch($import_type){
            ...
            case "CUSTOMER":
                $response = $this->import_file($import_type, $import_file);
                if($response['import_status'] == true){
                    $user_api = new CustomerAPI();
                    $request = request();
                    foreach ($response['data'] as $customer_array_item) {
                        $request->merge($customer_array_item);
                        $customer_api->store($request);
                    }
                    unset($response['data']);
                }
            break;
    ...
    public function import_file($import_type, $import_file){
        ...
        switch($import_type){
            ...
            break;
            case "CUSTOMER":
                $valid_template = $this->validate_template("CUSTOMER", $import_file);
            break;
            case "STORE":
        ...
        if($valid_template){
            ...
            foreach($excel_data as $key => $excel_data_item){
                switch($import_type){
                    ...
                    break;
                    case "CUSTOMER":
                        $validate_response = $this->validate_customer_data($excel_data_item);
                    break;
                    case "STORE":
    ...
    public function validate_template($template_type, $import_file){
        ...
        switch($template_type){
            ...
            break;
            case "CUSTOMER":
                $template_format = Config::get('constants.upload.imports.customer_format');
            break;
            case "STORE":
    ...
    public function validate_customer_data($excel_data_item)
    {
        $response = [];
        $data = [];
        $error_array  = [];
        ...
        $email          = $excel_data_item['email'];
        $contact_number = $excel_data_item['contact_number'];
        $role_id      = $excel_data_item['role_id'];
        $status         = $excel_data_item['status'];
        $store_id    = $excel_data_item['store_id'];

        $validator = Validator::make($excel_data_item, [
            'fullname' => $this->get_validation_rules("fullname", true),
            'email' => $this->get_validation_rules("email", true),
            'contact_number' => $this->get_validation_rules("phone", true),
            'role_id' => $this->get_validation_rules("ids", true),
            'status' => $this->get_validation_rules("filled",true),
            //'store_codes_csv' => $this->get_validation_rules("filled",true)
        ]);
        ...
        $role_data = RoleModel::select('slack', 'id')->where('id', '=', $role_id)->resolveSuperAdminRole()->active()->first();
        if (!$role_data) {
            $error_array[] = 'Invalid role code provided';
        }
        ...
        if ($store_id != "") {
            $store_data = StoreModel::select('slack', 'id')->where('id', '=', $store_id)->first();
            if ($store_data) {
                $error_array[] = 'Store Invalid';
            }
        }
