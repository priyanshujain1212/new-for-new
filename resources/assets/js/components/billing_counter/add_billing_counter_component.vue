        ...
        <div class="col-md-12">
            ...
            <form @submit.prevent="submit_form" class="mb-3">
                ...
                <div class="d-flex flex-wrap mb-4">
                    <div class="mr-auto">
                        <span class="text-title" v-if="billing_counter_slack == ''">{{ $t("Add Particular") }}</span>
                        <span class="text-title" v-else>{{ $t("Edit Particular") }}</span>
                    </div>
                    <div class="">
                ...
                <div class="form-row mb-2">
                    ...
                    <div class="form-group col-md-3">
                        <label for="billing_counter_name">{{ $t("Particular Name") }}</label>
                        <input type="text" name="billing_counter_name" v-model="billing_counter_name" v-validate="'required|max:150'" class="form-control form-control-custom" :placeholder="$t('Please enter particulars name')"  autocomplete="off">
                    ...
                    <div class="form-group col-md-3">
                        <label for="particular_to">{{ $t("Particular to") }}</label>
                        <select name="particular_to" v-model="particular_to" v-validate="'required'" class="form-control form-control-custom custom-select">
                            <option value="">Choose Particular for..</option>
                            <option v-for="(particular_to_item, index) in particular_to_master_list" v-bind:value="particular_to_item" v-bind:key="index">
                                {{ particular_to_item }}
                            </option>
                        </select>
                        <span v-bind:class="{ 'error' : errors.has('particular_to') }">{{ errors.first('particular_to') }}</span> 
                    </div>
                </div>
