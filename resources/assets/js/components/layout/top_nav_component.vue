<template>
    <nav class="navbar navbar-expand-lg top-nav p-2">
        <div class="container-fluid">
            ...
            <div id="menu-toggle" class="menu-toggler mr-auto" v-if="!order_page">
                <div class="bar-1"></div>
                <div class="bar-2"></div>
                <div class="bar-3"></div>
            </div>
            ...
            <button class="navbar-toggler dropdown-toggle" type="button" data-toggle="collapse" data-target="#small_menu_toogler" aria-controls="small_menu_toogler" aria-expanded="false" aria-label="Toggle actions">
                <img :src="logged_user_data?.profile_image || logged_customer_data?.profile_image" class="d-inline-block rounded-circle mr-2 top-nav-profile" alt="" />
            </button>
            ...
            <div class="collapse navbar-collapse" id="small_menu_toogler">
                ...
                <ul class="navbar-nav ml-auto mt-lg-0 pt-1" v-if="user_slack || customer_slack">
                    ...
                    <li class="nav-item text-right pl-md-4 pl-lg-4 pl-xl-4" v-if="new_order_access">
...
export default {
    ...
    data() {
        return {
            stores: this.logged_user_data?.user_stores || this.logged_customer_data?.customer_stores || '',
            selected_store: this.logged_user_data?.selected_store || this.logged_customer_data?.selected_store || '',
            quick_link_array: this.quick_links || [],
            unread_notifications_data: this.logged_user_data?.unread_notifications || this.logged_customer_data?.unread_notifications || [],
            all_notifications_link_data: this.logged_user_data?.all_notifications_link || this.logged_customer_data?.all_notifications_link || '',
            profile_link: this.logged_user_data?.profile_link || this.logged_customer_data?.profile_link || '',
            logout_link: this.logged_user_data?.logout_link || this.logged_customer_data?.logout_link || '',
            user_fullname: this.logged_user_data?.fullname || this.logged_customer_data?.fullname || ''
        }
    },
    props: {
        user_slack: String,
        logged_user_data: Object,
        navbar_logo: String,
        quick_links: Array,
        order_page: Boolean,
        logged_customer_data: Object,
        customer_slack: String,
    },
    mounted() {
        console.log('Top Navigation loaded');
        console.log(this.logged_customer_data);
    },
        methods: {
