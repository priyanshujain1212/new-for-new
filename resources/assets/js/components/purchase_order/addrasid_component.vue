        ...
        <div class="col-md-12">
            <form @submit.prevent="submit_form" class="mb-3">
                ...
                <div class="d-flex flex-wrap mb-4">
                    <div class="mr-auto">
                        <span class="text-title" v-if="purchase_order_slack == ''">{{ $t("Add New Rasid") }}</span>
                        <span class="text-title" v-else>{{ $t("Edit Rasid ") }} </span>
                    </div>
                    <div class="">
                ...
                <div class="form-row mb-2">
                    <div class="form-group col-md-3">
                        <label for="po_number">{{ $t("Rasid Number") }}</label>
                        <div v-if="purchase_order_slack == ''">
                            ...
                            <span v-bind:class="{ 'error': errors && errors.has('po_number') }">
                             {{ errors && errors.first('po_number') }}</span>
                    </div>
                        <div v-else>
                    ...
                    <div class="form-group col-md-3">
                            ...
                            <div v-if="purchase_order_slack == ''">
                                <v-select 
                                    ...
                                    :placeholder="$t('Please choose customer')" 
                                    autocomplete="off" 
                                    v-model="selectedCustomerslack" 
                                    :options="formattedCustomerList" 
                                    label="displayText" 
                                    @search="load_customers" 
                                    ref="customer"
                                ></v-select>
                                <span v-bind:class="{ 'error': errors && errors.has('customer') }">
                                    {{ errors && errors.first('customer') }}</span>
                            </div>
                            <div v-else>
                                <div class="ml-2" style="font-size: 1.5rem" value="customer_code">
                                    <span for="customer">{{ $t(customer_code) }}{{ $t(" - ") }}{{ $t(customer_name) }}</span>
                                </div>
                            </div>
                        </div>

                    <div class="form-group col-md-3">
                        <label for="po_reference">{{ $t("Reference Rasid Number (if any)") }}</label>
                        <input type="text" name="po_reference" v-model="po_reference" v-validate="'max:30'" class="form-control form-control-custom" :placeholder="$t('Reference Rasid Number')"  autocomplete="off">
                    </div>
                </div>
                <div class="form-row mb-2">
                    <div class="form-group col-md-3">
                        <label for="order_date">{{ $t(" Date") }}</label>
                        <DatePicker :format="date.format" :lang='date.lang' v-model="order_date" v-validate="'date_format:yyyy-MM-dd'" input-class="form-control form-control-custom  bg-white" ref="order_date" name="order_date" :placeholder="$t('Please enter rasid Date')" autocomplete="off"></DatePicker>
                        <span v-bind:class="{ 'error': errors && errors.has('order_date') }">
                            {{ errors && errors.first('order_date') }}</span> 
                    </div>
                    <div class="form-group col-md-3">
                        ...
                        <div v-if="!purchase_order_slack">  <!-- Check if purchase_order_slack is empty -->
                            ...
                            <span v-bind:class="{ 'error': errors && errors.has('payment_type') }">
                                {{ errors && errors.first('payment_type') }}</span>
                        </div>
                        <div v-else>
                ...
                <div class="form-row mb-2">
                    <div class="form-group col-md-6">
                        ...
                        <span v-bind:class="{ 'error': errors && errors.has('terms') }">
                            {{ errors && errors.first('terms') }}</span>
                    </div>
                </div>
            </form>
        </div>
